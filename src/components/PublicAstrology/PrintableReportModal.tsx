...
import { Download, Printer, X, Sparkles, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden text-slate-100 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex flex-wrap items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-950 gap-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              id="btn-print-report"
              onClick={handlePrint}
              className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
            >
            ...
            <button
              ...
              className="px-4 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-semibold flex items-center gap-1.5 shadow-lg shadow-indigo-500/25 transition cursor-pointer"
            >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-950/60 font-sans">
          {/* Header Banner */}
          <div className="p-6 bg-gradient-to-r from-indigo-950 via-slate-900 to-indigo-950 border border-indigo-800/40 rounded-2xl flex flex-wrap items-center justify-between gap-4">
            <div>
              <span className="text-[11px] font-bold text-amber-400 uppercase tracking-widest block mb-1">
                Official Natal Kundali Document
              </span>
              <h2 className="text-2xl font-black text-white">{chartData.subjectName}</h2>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 text-xs">
              {chartData.planets.map(p => (
                <div key={p.name} className="p-2.5 bg-slate-950 rounded-lg border border-slate-800/80">
                  <div className="flex items-center justify-between text-slate-400 text-[11px]">
                    <span className="font-bold text-white flex items-center gap-1">
                      <span className="text-indigo-400">{p.symbol}</span> {getPlanetName(p.name, selectedLanguage)}
                    </span>
                    <span className="text-indigo-400 font-semibold">{t('house')} {p.house}</span>
                  </div>
                  <div className="font-semibold text-amber-400 mt-1">
                    {getSignName(p.sign, selectedLanguage)} <span className="font-mono text-slate-300 text-[10px]">({p.formattedDegrees})</span>
                  </div>
          ...
          <div className="bg-slate-900 border border-indigo-900/40 rounded-xl p-4 space-y-3">
            <h4 className="text-sm font-bold text-indigo-300 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-indigo-400" /> {t('predictionsTitle')}
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
              {/* Weekly */}
              <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-amber-400">{t('weeklyForecast')}</span>
                ...
                <div className="text-[10px] text-slate-400 border-t border-slate-800/80 pt-1.5">
                  <span className="text-indigo-300 font-semibold">Career Tip:</span> {chartData.interpretations?.predictions?.weekly?.careerAndMoney?.actionableTip || 'Schedule major presentations mid-week.'}
                </div>
              </div>
              ...
              <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-emerald-400">{t('monthlyForecast')}</span>
              ...
              <div className="p-3 bg-slate-950 rounded-lg border border-slate-800 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-cyan-400">{t('yearlyForecast')}</span>
                ...
                <div className="text-[10px] text-slate-400 border-t border-slate-800/80 pt-1.5">
                  <span className="text-cyan-300 font-semibold">Strategy:</span> {chartData.interpretations?.predictions?.yearly?.careerAndMoney?.actionableTip || 'Focus on high-leverage assets and spiritual peace.'}
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900 border border-amber-900/40 rounded-xl p-4">
            ...
            <div className="space-y-3">
              {chartData.interpretations.gemstoneRecommendations.map((gem, i) => (
                <div key={i} className="p-3 bg-slate-950 rounded-lg border border-slate-800 text-xs flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    <p className="text-amber-400/90 text-[11px] font-mono mt-1 italic">
                      Mantra: "{gem.mantra}"
                    </p>
                  </div>
                  <div className="text-right text-xs">
                    <span className="px-2 py-0.5 bg-emerald-950 text-emerald-300 border border-emerald-800 rounded font-semibold text-[11px]">
                      {gem.suitability}
                    </span>
                    <div className="text-slate-400 text-[11px] mt-1">
                      Finger: <span className="text-white font-medium">{gem.finger}</span>
                    </div>
                  </div>
                </div>
              ))}
